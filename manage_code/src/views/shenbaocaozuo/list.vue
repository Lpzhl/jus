	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('shenbaocaozuo','新增')">
						新增
					</el-button>
					<el-button class="del_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('shenbaocaozuo','删除')">
						删除
					</el-button>
				</div>
			</div>
			<el-table
				...
				<el-table-column label="操作" class-name="operation-cell" width="300"  :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button class="view_btn" type="info" v-if=" btnAuth('shenbaocaozuo','查看')" @click="infoClick(scope.row.id)">
							详情
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(scope.row.id,scope.row)" v-if=" btnAuth('shenbaocaozuo','修改')">
							修改						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(scope.row.id,scope.row)"  v-if="btnAuth('shenbaocaozuo','删除')">
							删除						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('shenbaocaozuo','申报回执')" type="success" @click="shenbaohuizhiCrossAddOrUpdateHandler(scope.row,'cross','','','','')">
							申报回执
						</el-button>
					</template>
				</el-table-column>
		...
		<shenbaohuizhiFormModel ref="shenbaohuizhiFormModelRef" @formModelChange="formModelChange"></shenbaohuizhiFormModel>
	</div>
</template>
<script setup>
	...
	import shenbaohuizhiFormModel from '@/views/shenbaohuizhi/formModel'
	const shenbaohuizhiFormModelRef = ref(null)
    const shenbaohuizhiCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
