	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('ziliaoshangchuan','新增')">
						新增
					</el-button>
					<el-button class="del_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('ziliaoshangchuan','删除')">
						删除
					</el-button>
				</div>
			</div>
			<el-table
				...
				ref="table"
				v-if="btnAuth('ziliaoshangchuan','查看')"
				:data="list"
				...
				<el-table-column label="税务合同" min-width="140" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button v-if="scope.row.shuiwuhetong" size="small" @click="download(scope.row.shuiwuhetong)">下载</el-button>
						<el-button v-else disabled link size="small">无</el-button>
					</template>
				</el-table-column>
				<el-table-column label="附件凭证" min-width="140" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button v-if="scope.row.fujianpingzheng" size="small" @click="download(scope.row.fujianpingzheng)">下载</el-button>
						<el-button v-else disabled link size="small">无</el-button>
					</template>
				</el-table-column>
				...
				<el-table-column label="审核" v-if="btnAuth('ziliaoshangchuan','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" class-name="operation-cell" width="300"  :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button class="view_btn" type="info" v-if=" btnAuth('ziliaoshangchuan','查看')" @click="infoClick(scope.row.id)">
							详情
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(scope.row.id,scope.row)" v-if=" btnAuth('ziliaoshangchuan','修改')">
							修改						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(scope.row.id,scope.row)"  v-if="btnAuth('ziliaoshangchuan','删除')">
							删除						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('ziliaoshangchuan','归档')" type="success" @click="ziliaoguidangCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','请勿重复操作')">
							归档
						</el-button>
					</template>
				</el-table-column>
		...
		<ziliaoguidangFormModel ref="ziliaoguidangFormModelRef" @formModelChange="formModelChange"></ziliaoguidangFormModel>
	</div>
</template>
<script setup>
	...
	import ziliaoguidangFormModel from '@/views/ziliaoguidang/formModel'
	const ziliaoguidangFormModelRef = ref(null)
    const ziliaoguidangCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
