<template>
    <div class="list-page">
		...
		<div class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" v-if="btnAuth('jichushuju','新增')" @click="addClick">新增</el-button>
			</div>
            <div class="chartBtn-row">
                <el-button class="chart_btn" type="warning" @click="echartClick1" v-if="btnAuth('jichushuju','税务变化')">
                    税务变化
                </el-button>
                <el-button class="chart_btn" type="warning" @click="echartClick2" v-if="btnAuth('jichushuju','申报进度')">
                    申报进度
                </el-button>
            </div>
		</div>
                ...
                <div class="table_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="申报进度" :resizable='true' align="center" header-align="center">
							<template #default="scope">
    ...
    <el-dialog v-model="echartVisible" modal-class="chart-dialog-modal" class="chart-dialog" title="统计图" width="70%">
        ...
        <div v-if="echartActive==1" class="chart-wrapper">
            <div class="chart" id="nianfenEchart1" style="width:100%;height:600px;"></div>
        </div>
        <div v-if="echartActive==2" class="chart-wrapper">
            <div class="chart" id="shenbaojinduEchart2" style="width:100%;height:600px;"></div>
        </div>
        <template #footer>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/jichushujuAdd')
	}
	//判断是否从个人中心跳转
